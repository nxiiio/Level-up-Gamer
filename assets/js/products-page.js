...
document.addEventListener("DOMContentLoaded", () => {
  ...
  card.innerHTML = `
        <div class="product-image">
          <a href="producto.html?code=${p.code}" aria-label="${p.name}" style="display:block;width:100%;height:100%;border-radius:10px;overflow:hidden">
            <img src="../${p.img}" alt="${p.name}" style="width:100%;height:100%;object-fit:cover;border-radius:10px">
          </a>
        </div>
        <div class="product-info">
          <div class="product-title">
            <a href="producto.html?code=${p.code}" style="color:#fff;text-decoration:none">${p.name}</a>
          </div>
      ...
      card.onclick = () => {
        location.href = `producto.html?code=${p.code}`;
      };
      card.onkeydown = (e) => {
